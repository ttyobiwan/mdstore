defmodule MdstoreWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, MdstoreWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      MdstoreWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "require_admin_user/2" do
    ...
    test "does not redirect if user is an admin", %{conn: conn, user: user} do
      {:ok, user} = Mdstore.Repo.update(Ecto.Changeset.change(user, %{is_admin: true}))
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        MdstoreWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
